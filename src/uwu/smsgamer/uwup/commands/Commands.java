					...
					if (args[0].equalsIgnoreCase(p.getName())) {
						...
						if (args[1].startsWith("n") || args[1].startsWith("f")) {
							...
							if (args[2].startsWith("h")) {
								...
								if (Main.instance.getConfig().get("Punishments.Hax.Level." + p.getUniqueId()) == null) {
									Main.instance.getConfig().set("Punishments.Hax.Level." + p.getUniqueId(),
									...
									if (Main.instance.getConfig()
										...
										p.kickPlayer(
												Vars.prefix + "Kicked for hacking by a moderator. Hacking punishments: "
														+ Main.instance.getConfig().getInt(
							...
							if (args[2].startsWith("s")) {
								...
								if (Main.instance.getConfig()
									...
									Main.instance.getConfig().set("Punishments.Spam.Level." + p.getUniqueId(),
											Main.instance.getConfig()
									...
									if (Main.instance.getConfig()
										...
										Main.instance.getConfig().set("Punishments.Spam.Level." + p.getUniqueId(),
												Main.instance.getConfig()
														.getInt(("Punishments.Spam.Level." + p.getUniqueId())) + 1);
										Main.instance.getConfig().set("Punishments.Spam.Muted." + p.getUniqueId(),
												true);
							...
							if (args[2].startsWith("o")) {
								...
								if (Main.instance.getConfig().get("Punishments.Other.Level." + p.getName()) == null) {
									...
									Main.instance.getConfig().set("Punishments.Other.Level." + p.getName(),
											Main.instance.getConfig().getInt("Punishments.Other.Level." + p.getName())
													+ 1);
							...
							if (args[2].startsWith("s")) {
								if (Main.instance.getConfig().get("Punishments.Spam.Level." + p.getName()) == null) {
									Main.instance.getConfig().set("Punishments.Spam.Level." + p.getName(),
											Main.instance.getConfig().getInt(("Punishments.Spam.Level." + p.getName()))
													+ 1);
									p.sendMessage(Vars.prefix + "You have been warned for spam.");
								} else {
									Main.instance.getConfig().set("Punishments.Spam.Level." + p.getName(),
